...
export const getEventAttendees =
  (eventID: string) => async (dispatch: AppDispatch) => {
    ...
    const eventAttendeedRef = collection(
      firestoreV9,
      "events",
      eventID,
      FIREBASE_COLLECTIONS.privateUnverifiedAttendees
    );
...
export const manageEventSlice = createSlice({
  ...
  reducers: {
    ...
    storeAttendees: (
      state,
      action: PayloadAction<EventDetails["attendees"]>
    ) => {
      if (action.payload) {
    ...
    storeAttendeed: (
      state,
      action: PayloadAction<EventDetails["attendeed"]>
    ) => {
